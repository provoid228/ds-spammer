...
def get_user_input():
    ...
    if msg_input_method == 1:
        message_file = input('TXT файл с сообщениями: ')
        msg_folders = {token: message_file for token in tokens}
    else:
        msg_folders = {}
        for token in tokens:
            token_key = token.split(':')[0] if ':' in token else token
            msg_folders[token_key] = input(f'Перетяните файл с сообщениями для {token_key}: ')
...
# Message handling and Discord interactions
def rand_msg(msg_folder, take_msgs):
    messages = load_messages(msg_folder)
    return messages.pop(0) if take_msgs == 1 else messages.pop(randint(0, len(messages)-1))
...
def main_thread(token, config):
    ...
    while True:
        try:
            lock.acquire()
            message = rand_msg(config['msg_folders'][token], config['take_msgs'])
            lock.release()

            typing_delay = randint(*map(int, config['typing_delay'].split('-'))) if '-' in config['typing_delay'] else int(config['typing_delay'])
            logger.info(f'Имитирую печатание для [{username}] в течение {typing_delay} сек')

            message_id = send_message(session, chat_id, message, typing_delay, config['delay_every_msg'])
            if message_id:
                logger.success(f'Сообщение [{message}] от [{username}] успешно отправлено')
